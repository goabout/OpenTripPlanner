...
public class TestParkAndRide extends TestCase {
    ...
    protected void setUp() throws Exception {
        ...
        Edge driveOnly = new PlainStreetEdge(A, B, GeometryUtils.makeLineString(0.000, 45, 0.001, 45),
                "AB street", 87, StreetTraversalPermission.CAR, false);

        @SuppressWarnings("unused")
        Edge walkAndBike = new PlainStreetEdge(B, C, GeometryUtils.makeLineString(0.001, 45, 0.002,
                45), "BC street", 87, StreetTraversalPermission.PEDESTRIAN_AND_BICYCLE, false);

        @SuppressWarnings("unused")
        Edge walkOnly = new PlainStreetEdge(C, D, GeometryUtils.makeLineString(0.002, 45, 0.003,
    ...
    public void testBike() throws Exception {
        ...
        RoutingRequest options = new RoutingRequest(new TraverseModeSet("BICYCLE,WALK"));
        options.setBikeParkAndRide(true);
        options.freezeTraverseMode();
        ...
        options.setBikeParkAndRide(false);
        options.setRoutingContext(graph, B, D);
        ...
        options = new RoutingRequest(new TraverseModeSet("BICYCLE,WALK"));
        options.setBikeParkAndRide(true);
        options.setRoutingContext(graph, B, D);
        tree = aStar.getShortestPathTree(options);
        path = tree.getPath(D, false);
